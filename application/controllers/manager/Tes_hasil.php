        ...
        if(!empty($tes_id) AND !empty($grup_id) AND !empty($waktu) AND !empty($urutkan) AND !empty($status)){
            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
					$jml_max = $this->cbt_tes_model->get_by_kolom_limit('tes_id', $temp->tes_id, 1)->row()->tes_max_score;
				$nilai = round(($temp->nilai/$jml_max),2)*100;
                    ...
                    $worksheet->setCellValueByColumnAndRow(6, $row, $nilai);
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			if(empty($temp->nilai)){
				...
				$jml_max = $this->cbt_tes_model->get_by_kolom_limit('tes_id', $temp->tes_id, 1)->row()->tes_max_score;
				$record[] = round(($temp->nilai/$jml_max),2)*100;
			}
