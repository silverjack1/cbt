	            ...
	            if($this->cbt_tes_user_model->count_by_user_tes($user_id, $temp->tes_id)->row()->hasil>0){
	            	...
	            	if($tanggal<$tanggal_tes AND $query_test_user->tesuser_status!=4){
		            	...
		            	if($temp->tes_results_to_users==1){
							$jmlsoal = $this->cbt_tes_model->get_by_kolom_limit('tes_id', $temp->tes_id, 1)->row()->tes_max_score;
							$benar = $this->cbt_tes_soal_model->get_nilai($query_test_user->tesuser_id)->row()->hasil;
							$hitung = round(($benar/$jmlsoal),2)*100; 
							$record[] = $hitung;
		            	}else{
		            		$record[] = '';
		            	}
